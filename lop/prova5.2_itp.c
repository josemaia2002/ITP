...
int main(){
    ...
    for (int i = 0; i < control_var_aprov; i++){
        if(i == (control_var_aprov - 1)){
            printf("Aprovados: %d (%.1lf) ", chamada[aprovados[i]], media[aprovados[i]]);
        }
        else {
            printf("Aprovados: %d (%.1lf), ", chamada[aprovados[i]], media[aprovados[i]]);
    ...
    for (int i = 0; i < control_var_rec; i++){
        if(i == (control_var_rec - 1)){
            printf("Recuperacao: %d (%.1lf) ", chamada[rec[i]], media[rec[i]]);
        }
        else {
            printf("Recuperacao: %d (%.1lf), ", chamada[rec[i]], media[rec[i]]);
        }
    ...
    for (int i = 0; i < control_var_reprov; i++){
        if(i == (control_var_reprov - 1)){
            printf("Reprovados: %d (%.1lf) ", chamada[reprovados[i]], media[reprovados[i]]);
        }
        else {
            printf("Reprovados: %d (%.1lf), ", chamada[reprovados[i]], media[reprovados[i]]);
        }
